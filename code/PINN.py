...
class SVE:
    ...
    def __init__(self,
        ...
        self.loss_BCs = self.loss_BC_Q + 1 * self.loss_BC_Z
    ...
    def compute_loss_landscape(self, grid_size=51, scale_range=0.3, feed_dict=None):
        ...
        # 保存数据到文件
        np.savez('结果/插值/loss_landscape_data.npz', alphas=alphas, betas=betas, losses=losses)
    ...
    def fun_sed(self, Q, S, Z_t, S_t, S_x, Q_x):
        pde3_loss = [0]
        for i in range(7):
            s = S * self.delta_S[:, i:i + 1] / 1  # 第i组悬沙含沙量
    ...
    def train(self, num_epochs):
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                Q_pred, Z_pred, S_pred, A0_pred = self.predict(self.X_star[:, 0:1], self.X_star[:, 1:2], self.x_A, self.x_B, self.x_w, self.x_a, self.x_delta_S)
                ...
                with open("结果/插值/train_loss.txt", 'w') as train_los:
                    train_los.write(str(train_loss))
                with open("结果/插值/train_Q_error.txt", 'w') as train_ac:
    ...
    def predict(self, x_test, t_test, A_test, B_test, w_test, a_test, delta_S_test):  # 网络预测输出5个值，已改

        tf_dict = {self.x_u_tf: x_test, self.t_u_tf: t_test,
                   self.x_A_tf: A_test, self.x_B_tf: B_test,
                   self.x_w_tf: w_test, self.x_a_tf: a_test,
                   self.x_delta_S_tf: delta_S_test,
                   self.x_h_tf: x_test, self.t_h_tf: t_test}
