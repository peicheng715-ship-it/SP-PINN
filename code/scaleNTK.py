...
class SVE:
    ...
    def __init__(self,
        ...
        self.Q_r_ntk_pred, self.Z_r_ntk_pred, self.S_r_ntk_pred, self.A0_r_ntk_pred = self.net_uh(
            ...
            self.x_A_r_ntk_tf,
            self.x_B_r_ntk_tf, self.x_w_r_ntk_tf,
    ...
    def compute_loss_landscape(self, grid_size=51, scale_range=0.3, feed_dict=None):
        ...
        # 保存数据到文件
        np.savez('结果/试验48/缩放NTK全参数/loss_landscape_data.npz', alphas=alphas, betas=betas, losses=losses)
    ...
    def net_f_ntk(self, x_f, t_f, x_A_tf, x_B_tf, x_w_tf, x_a_tf, x_delta_S_tf):
        X_f = 2.0 * (tf.concat([x_f, t_f, x_A_tf, x_B_tf, x_w_tf, x_a_tf,
                                x_delta_S_tf], 1) - self.lb) / (self.ub - self.lb) - 1.0
        output4 = self.neural_net(X_f, self.weights, self.biases)
        Q = output4[:, 0:1]
    ...
    def train(self, num_epochs, log_NTK_freq=10, update_lam=True):
        ...
        excel_path = "结果/试验48/缩放NTK全参数/log.xlsx"
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                np.savez(
                    '结果/试验48/缩放NTK全参数/best_predictions.npz',
                    Q_pred=Q_pred,
                    Z_pred=Z_pred,
                    S_pred=S_pred,
                    A0_pred=A0_pred,
                ...
                with open("结果/试验48/缩放NTK全参数/train_loss.txt", 'w') as train_los:
                    train_los.write(str(train_loss))

                with open("结果/试验48/缩放NTK全参数/train_Q_error.txt", 'w') as train_ac:
