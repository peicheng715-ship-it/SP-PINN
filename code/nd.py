...
class SVE:
    ...
    def __init__(self,
        ...
        # ========== Predictions for NTK ==========
        # # 关键：这些都是预测值，没有真实值参与！
        # self.Q_data_ntk_pred, self.Z_data_ntk_pred, self.S_data_ntk_pred, self.A0_data_ntk_pred = (
        #                                                       self.net_uh(self.x_data_ntk_tf, self.t_data_ntk_tf,
        #                                                                   self.x_A_data_ntk_tf,
        #                                                                   self.x_B_data_ntk_tf, self.x_w_data_ntk_tf,
        #                                                                   self.x_a_data_ntk_tf, self.x_delta_S_data_ntk_tf))
        #
        # self.Q_ics_ntk_pred, self.Z_ics_ntk_pred, self.S_ics_ntk_pred, self.A0_ics_ntk_pred = self.net_uh(
        #     self.x_ics_ntk_tf, self.t_ics_ntk_tf,
        #     self.x_A_ics_ntk_tf,
        #     self.x_B_ics_ntk_tf, self.x_w_ics_ntk_tf,
        #     self.x_a_ics_ntk_tf, self.x_delta_S_ics_ntk_tf)
        # self.Q_bcs_ntk_pred, self.Z_bcs_ntk_pred, self.S_bcs_ntk_pred, self.A0_bcs_ntk_pred = self.net_uh(
        #     self.x_bcs_ntk_tf, self.t_bcs_ntk_tf,
        #     self.x_A_bcs_ntk_tf,
        #     self.x_B_bcs_ntk_tf, self.x_w_bcs_ntk_tf,
        #     self.x_a_bcs_ntk_tf, self.x_delta_S_bcs_ntk_tf)
        self.Q_r_ntk_pred, self.Z_r_ntk_pred, self.S_r_ntk_pred, self.A0_r_ntk_pred = self.net_uh(
            self.x_r_ntk_tf, self.t_r_ntk_tf,

            self.x_a_r_ntk_tf, self.x_delta_S_r_ntk_tf)
    ...
    def compute_loss_landscape(self, grid_size=51, scale_range=0.3, feed_dict=None):
        ...
        # 保存数据到文件
        np.savez('结果/试验48/缩放NTK无ABw/loss_landscape_data.npz', alphas=alphas, betas=betas, losses=losses)
    ...
    # 输出改为5个变量
    # 输入增加所有方程参数
    def net_uh(self, x, t, a, delta_S):
        X = 2.0 * (tf.concat([x, t,  a, delta_S], 1) - self.lb) / (
                self.ub - self.lb) - 1.0
    ...
    # 已修改
    def net_f(self, x_f, t_f, x_a_tf, x_delta_S_tf):
        X_f = 2.0 * (tf.concat([x_f, t_f, x_a_tf,
    ...
    def net_f_ntk(self, x_f, t_f, x_a_tf, x_delta_S_tf):
        X_f = 2.0 * (tf.concat([x_f, t_f, x_a_tf,
                                x_delta_S_tf], 1) - self.lb) / (self.ub - self.lb) - 1.0
        output4 = self.neural_net(X_f, self.weights, self.biases)
        Q = output4[:, 0:1]
    ...
    def train(self, num_epochs, log_NTK_freq=10, update_lam=True):
        ...
        excel_path = "结果/试验48/缩放NTK无ABw/log.xlsx"
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                np.savez(
                    '结果/试验48/缩放NTK无ABw/best_predictions.npz',
                    Q_pred=Q_pred,
                    Z_pred=Z_pred,
                    S_pred=S_pred,
                    A0_pred=A0_pred,
                ...
                with open("结果/试验48/缩放NTK无ABw/train_loss.txt", 'w') as train_los:
                    train_los.write(str(train_loss))

                with open("结果/试验48/缩放NTK无ABw/train_Q_error.txt", 'w') as train_ac:
            ...
            if it % log_NTK_freq == 0 and update_lam:
                ...
                # Sample points for NTK computation (可以与训练batch不同)
                # X_data_ntk, _ = self.fetch_minibatch(self.X_u_obs, self.kernel_size)
                # x_A_data_ntk,  _ = self.fetch_minibatch(self.x_A_obs, self.kernel_size)
                # x_B_data_ntk, _ = self.fetch_minibatch(self.x_B_obs, self.kernel_size)
                # x_delta_S_data_ntk, _ = self.fetch_minibatch(self.x_delta_S_obs, self.kernel_size)
                # x_w_data_ntk, _ = self.fetch_minibatch(self.x_w_obs, self.kernel_size)
                # x_a_data_ntk, _ = self.fetch_minibatch(self.x_a_obs, self.kernel_size)

                # X_ics_ntk, _ = self.fetch_minibatch(self.X_Z_BC, self.kernel_size)
                # x_A_ics_ntk, _ = self.fetch_minibatch(self.x_Z_BC_A, self.kernel_size)
                # x_B_ics_ntk, _ = self.fetch_minibatch(self.x_Z_BC_B, self.kernel_size)
                # x_delta_S_ics_ntk, _ = self.fetch_minibatch(self.x_Z_BC_delta_S, self.kernel_size)
                # x_w_ics_ntk, _ = self.fetch_minibatch(self.x_Z_BC_w, self.kernel_size)
                # x_a_ics_ntk, _ = self.fetch_minibatch(self.x_Z_BC_a, self.kernel_size)
                #
                # X_bcs_ntk, _ = self.fetch_minibatch(self.X_u_BC, self.kernel_size)
                # x_A_bcs_ntk, _ = self.fetch_minibatch(self.x_BC_A, self.kernel_size)
                # x_B_bcs_ntk, _ = self.fetch_minibatch(self.x_BC_B, self.kernel_size)
                # x_delta_S_bcs_ntk, _ = self.fetch_minibatch(self.x_BC_delta_S, self.kernel_size)
                # x_w_bcs_ntk, _ = self.fetch_minibatch(self.x_BC_w, self.kernel_size)
                # x_a_bcs_ntk, _ = self.fetch_minibatch(self.x_BC_a, self.kernel_size)

                # X_res_ntk, _ = self.fetch_minibatch(self.X_f, 512)
                # x_A_res_ntk, _ = self.fetch_minibatch(self.x_A, 512)
                # x_B_res_ntk, _ = self.fetch_minibatch(self.x_B, 512)
                # x_delta_S_res_ntk, _ = self.fetch_minibatch(self.x_delta_S, 512)
                # x_w_res_ntk, _ = self.fetch_minibatch(self.x_w, 512)
                # x_a_res_ntk, _ = self.fetch_minibatch(self.x_a, 512)

                tf_dict_ntk = {
                    self.x_data_ntk_tf: self.X_u_obs[:, 0:1],
                    self.t_data_ntk_tf: self.X_u_obs[:, 1:2],
                    self.x_ics_ntk_tf: self.X_Z_BC[:, 0:1],
                    self.t_ics_ntk_tf: self.X_Z_BC[:, 1:2],
                    self.x_bcs_ntk_tf: self.X_u_BC[:, 0:1],
                    self.t_bcs_ntk_tf: self.X_u_BC[:, 1:2],
                    self.x_r_ntk_tf: self.X_f[:, 0:1],
                    self.t_r_ntk_tf: self.X_f[:, 1:2],
                    self.x_A_data_ntk_tf: self.x_A_obs,
                    self.x_B_data_ntk_tf: self.x_B_obs,
                    self.x_delta_S_data_ntk_tf: self.x_delta_S_obs,
                    self.x_w_data_ntk_tf: self.x_w_obs,
                    self.x_a_data_ntk_tf: self.x_a_obs,

                    self.x_A_ics_ntk_tf: self.x_Z_BC_A,
                    self.x_B_ics_ntk_tf: self.x_Z_BC_B,
                    self.x_delta_S_ics_ntk_tf: self.x_Z_BC_delta_S,
                    self.x_w_ics_ntk_tf: self.x_Z_BC_w,
                    self.x_a_ics_ntk_tf: self.x_Z_BC_a,

                    self.x_A_bcs_ntk_tf: self.x_BC_A,
                    self.x_B_bcs_ntk_tf: self.x_BC_B,
                    self.x_delta_S_bcs_ntk_tf: self.x_BC_delta_S,
                    self.x_w_bcs_ntk_tf: self.x_BC_w,
                    self.x_a_bcs_ntk_tf: self.x_BC_a,

                    self.x_A_r_ntk_tf: self.x_A,
                    self.x_B_r_ntk_tf: self.x_B,
                    self.x_delta_S_r_ntk_tf: self.x_delta_S,
                    self.x_w_r_ntk_tf: self.x_w,
                    self.x_a_r_ntk_tf: self.x_a,
