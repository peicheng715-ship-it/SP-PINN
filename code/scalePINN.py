...
class SVE:
    ...
    def __init__(self,
        ...
        self.dummy_tf = tf.placeholder(self.DTYPE, shape=(None, shared_layers[-1]))  # dummy variable for fwd_gradients
    ...
    def compute_loss_landscape(self, grid_size=51, scale_range=0.3, feed_dict=None):
        if feed_dict is None:
            ...
            feed_dict = {
                ...
                self.dummy_tf: np.ones((self.t_f.shape[0], self.share_layers[-1])),
            }
        ...
        # 保存数据到文件
        np.savez('结果/实验Z/有输入/缩放500/缩放/loss_landscape_data.npz', alphas=alphas, betas=betas, losses=losses)
    ...
    def train(self, num_epochs):
        ...
        excel_path = "结果/实验Z/有输入/缩放500/缩放/log.xlsx"
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                with open("结果/实验Z/有输入/缩放500/缩放/train_loss.txt", 'w') as train_los:
                    train_los.write(str(train_loss))
                with open("结果/实验Z/有输入/缩放500/缩放/train_Q_error.txt", 'w') as train_ac:
