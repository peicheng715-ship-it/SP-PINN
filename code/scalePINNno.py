...
class SVE:
    ...
    def compute_loss_landscape(self, grid_size=51, scale_range=0.3, feed_dict=None):
        ...
        # 保存数据到文件
        np.savez('结果/实验Z/无输入/缩放500/缩放/最佳轮次/loss_landscape_data.npz', alphas=alphas, betas=betas, losses=losses)
    ...
    def train(self, num_epochs):
        ...
        excel_path = "结果/实验Z/无输入/缩放500/缩放/最佳轮次/log.xlsx"
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                np.savez(
                    '结果/实验Z/无输入/缩放500/缩放/最佳轮次/best_predictions.npz',
                    Q_pred=Q_pred,
                    Z_pred=Z_pred,
                    S_pred=S_pred,
                    A0_pred=A0_pred,
                ...
                with open("结果/实验Z/无输入/缩放500/缩放/最佳轮次/train_loss.txt", 'w') as train_los:
                    train_los.write(str(train_loss))

                with open("结果/实验Z/无输入/缩放500/缩放/最佳轮次/train_Q_error.txt", 'w') as train_ac:
